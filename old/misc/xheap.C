...
void _dbg_validate_heap ( const char * file, unsigned line )
{
  ...
  for ( curP = heap.Flink; !END_OF_LIST( &heap, curP ); curP = curP->Flink )
  {
    ...
    {
      TRACE2( "\n\n!!! %s(%u): ", file, line );
      TRACE2( "VALIDATE_HEAP(%p) failed; %s\n\n", curP + 1, heapErrMsg[err] );
      AbortProg();
    }
  }
                                   ...
                                   const char * file, unsigned line )
{
  ...
  {
    TRACE2( "\n\n!!! %s(%u): ", file, line );
    TRACE2( "VALIDATE_HEAP_PTR(%p) failed:%s\n\n", ptr, heapErrMsg[err] );
    AbortProg();
  }
  if (size != 0 && p->size != size)
  {
    TRACE2( "\n\n!!! %s(%u): ", file, line );
    TRACE3( "VALIDATE_HEAP_PTR(%p) size mismatch.\n"
            "size = %u, block size = %u\n\n", ptr, size, p->size );
    AbortProg();
  }
};
